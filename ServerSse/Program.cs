...
app.MapGet("/sse", async (HttpResponse response, CancellationToken requestAborted) =>
{
    McpServerOptions options = new()
    {
        ...
        {
            Tools = Config.ConfigureTools(),
            Resources = Config.ConfigureResources(),
            Prompts = Config.ConfigurePrompts(),
        },
        ProtocolVersion = "2024-11-05",
        ServerInstructions = "TESt",
        GetCompletionHandler = Config.ConfigureCompletion(),
    };
    ...
    await server.StartAsync(requestAborted);
    ...
    {
        //This is always triggered for SSE transport
...
public static class Config
{
        ...
        return new()
        {
            ...
            CallToolHandler = async (request, cancellationToken) =>
            {
                ...
                else if (request.Params?.Name == "sampleLLM")
                {
                    ...
                    var sampleResult = await request.Server.RequestSamplingAsync(
                        CreateRequestSamplingParams(prompt?.ToString() ?? "", "sampleLLM",
                    ...
                    return new CallToolResponse()
                    {
                        Content =
                        [
                            new Content() { Text = $"LLM sampling result: {sampleResult.Content.Text}", Type = "text" }
                        ]
                    };
                }
                else
    ...
    public static PromptsCapability ConfigurePrompts()
    {
        return new()
        {
    ...
    public static readonly HashSet<string> _subscribedResources = new();
    public static readonly object _subscribedResourcesLock = new();

    public static ResourcesCapability ConfigureResources()
    {
                ...
                if (request.Params?.Cursor is not null)
                {
                    try
                    {
                        var startIndexAsString =
                            Encoding.UTF8.GetString(Convert.FromBase64String(request.Params.Cursor));
    ...
    public static Func<RequestContext<CompleteRequestParams>, CancellationToken, Task<CompleteResult>>
        ConfigureCompletion()
    {
